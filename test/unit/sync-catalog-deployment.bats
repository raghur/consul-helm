...
@test "syncCatalog/Deployment: CONSUL_HTTP_TOKEN env variable created when global.acls.enabled=true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/sync-catalog-deployment.yaml \
      --set 'syncCatalog.enabled=true' \
...
@test "syncCatalog/Deployment: init container is created when global.acls.enabled=true" {
  cd `chart_dir`
...
@test "syncCatalog/Deployment: cross namespace policy is not added when global.acls.enabled=false" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/sync-catalog-deployment.yaml \
      --set 'syncCatalog.enabled=true' \
...
@test "syncCatalog/Deployment: cross namespace policy is added when global.acls.enabled=true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/sync-catalog-deployment.yaml \
      --set 'syncCatalog.enabled=true' \

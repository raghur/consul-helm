...
@test "enterpriseLicense/ClusterRole: rules are empty if global.acls.enabled and global.enablePodSecurityPolicies are false" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/enterprise-license-clusterrole.yaml  \
      --set 'server.enterpriseLicense.secretName=foo' \
...
@test "enterpriseLicense/ClusterRole: allows acl token when global.acls.enabled is true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/enterprise-license-clusterrole.yaml  \
      --set 'server.enterpriseLicense.secretName=foo' \

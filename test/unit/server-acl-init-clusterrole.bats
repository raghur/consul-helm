...
@test "serverACLInit/ClusterRole: enabled with global.acls.enabled=true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/server-acl-init-clusterrole.yaml  \
...
@test "serverACLInit/ClusterRole: disabled with server=false and global.acls.enabled=true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/server-acl-init-clusterrole.yaml  \
      --set 'global.acls.enabled=true' \
...
@test "serverACLInit/ClusterRole: enabled with client=false and global.acls.enabled=true" {
  cd `chart_dir`
  local actual=$(helm template \
      -x templates/server-acl-init-clusterrole.yaml  \
      --set 'global.acls.enabled=true' \
